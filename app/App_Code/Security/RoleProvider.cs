    ...
    public class ApplicationRoleProviderBase : RoleProvider
    {
        ...
        static ApplicationRoleProviderBase()
        {
            Statements[RoleProviderSqlStatement.AddUserToRole] = "\r\ninsert into UserRoles(UserID, RoleID) \r\nvalues (\r\n     (select UserID from User" +
                "s where UserName=@UserName) \r\n    ,(select RoleID from Roles where RoleName=@Rol" +
                "eName)\r\n    \r\n)";
            ...
            Statements[RoleProviderSqlStatement.DeleteRoleUsers] = "delete from UserRoles where RoleID in (select RoleID from Roles where RoleName = " +
                "@RoleName)";
            Statements[RoleProviderSqlStatement.GetAllRoles] = "select RoleName RoleName from Roles";
            Statements[RoleProviderSqlStatement.GetRolesForUser] = "\r\nselect Roles.RoleName RoleName from Roles Roles \r\ninner join UserRoles UserRole" +
                "s on Roles.RoleID = UserRoles.RoleID \r\ninner join Users Users on Users.UserID = " +
                "UserRoles.UserID\r\nwhere Users.UserName = @UserName";
            Statements[RoleProviderSqlStatement.GetUsersInRole] = "select UserName UserName from Users where UserID in (select UserID from UserRoles" +
            ...
            Statements[RoleProviderSqlStatement.IsUserInRole] = "\r\nselect count(*) from UserRoles\r\nwhere\r\n    UserID in (select UserID from Users " +
                "where UserName = @UserName) and \r\n    RoleID in (select RoleID from Roles where " +
                "RoleName = @RoleName)";
            Statements[RoleProviderSqlStatement.RemoveUserFromRole] = "\r\ndelete from UserRoles\r\nwhere\r\n   UserID in (select UserID from Users where User" +
                "Name = @UserName) and\r\n   RoleID in (select RoleID from Roles where RoleName = @" +
                "RoleName)";
            ...
            Statements[RoleProviderSqlStatement.FindUsersInRole] = "\r\nselect Users.UserName UserName from Users Users\r\ninner join UserRoles UserRoles" +
                " on Users.UserID= UserRoles.UserID \r\ninner join Roles Roles on UserRoles.RoleID " +
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (string.IsNullOrEmpty(name))
            	name = "FinsoftApplicationRoleProvider";
            if (string.IsNullOrEmpty(config["description"]))
            {
                config.Remove("description");
                config.Add("description", "Finsoft application role provider");
            }
