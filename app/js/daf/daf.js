...
(function () {
    ...
    _app.toAjaxData = function (data) {
        return _app.htmlEncode(JSON ? jsonStringify(data) : serialize(data));
    };
    ...
    _app.prototype = {
            ...
            if (method === 'row') {
                ...
                $(allFields).each(function (index) {
                    ...
                    if (!(name === 'sys_pk_' || name === 'Status' && index === allFields.length - 1)) {
                        ...
                        if (originalRow && originalRow[index] !== value)
                ...
                return dataView._collectFieldValues();
        ...
        dispose: function () {
            ...
            if (!_touch && !Sys.Application._disposing) {
        ...
        convertFieldValueToString: function (field, value) {
            ...
            return String.format('%js%{0}', serialize(value));
        },
                    ...
                    if (dataView && !dataView.get_isModal()) {
                        ...
                        if (result) {
                            ...
                            if (survey)
                                ...
                                dataView = _app.showModal(this, controller, view, commandName, commandArgument, __baseUrl, servicePath, [], { confirmContext: context, showSearchBar: that.get_showSearchBar(), tag: 'discard-changes-prompt-none' });
                            result = false;
                        }
                    }
            }
        ...
        _downloadFile: function (args) {
            ...
            $get('q', form).value = serialize(args.request);
            if (user && user.access_token)
                $get('t', form).value = user.access_token;
            if (args.actionArgs)
                $get('aa', form).value = _app.htmlEncode(serialize(args.actionArgs));
            form.submit();
        },
        ...
        executeCommand: function (args) {
            var that = this,
                ...
                rules = _app.rules[that._controller], handledByRule,
                actionPath = args.path, actionIDs,
                commandName = args.commandName,
                commandArgument = args.commandArgument;
        ...
        _notifyDesigner: function (changed) {
            if (__designer() && changed) {
                ...
                external.ExplorerNodeChanged(this.get_controller(), this.get_viewId(), lastArgs.CommandName, lastArgs.CommandArgument, serialize(lastArgs.Values));
            }
        },
        ...
        _validateFieldValueFormat: function (field, v, skipDateAdjustment) {
            ...
            switch (field.Type) {
                ...
                case 'Boolean':
                    try {
                        v.NewValue = isInstanceOfType(String, v.NewValue) ? Boolean.parse(v.NewValue) : v.NewValue;
                    }
                    catch (e) {
            ...
            for (i = 0; i < values.length; i++) {
                ...
                if (_touch) {
                    if (error && displayErrors && valid)
                        if (!_touch || _app.input.visible(field)) {
        ...
        _isActionAvailable: function (action, rowIndex) {
            ...
            var available = !lastCommand.length || lastCommand === that.get_lastCommandName() && (!lastArgument.length || lastArgument === that.get_lastCommandArgument());
            if (available) {
        ...
        search: function (args) {
            ...
            function convertValue(v) {
                ...
                return String.format('%js%{0}', serialize(v));
            }
            ...
            for (i = 0; i < currentFilter.length; i++) {
                ...
                while (m && (m[1].startsWith('~') || !(field.Index == field.AliasIndex && field.IsPrimaryKey && field.Hidden))) {
                    if (!first)
                        ...
                        sb.appendFormat('<span class="FilterElement" onclick="$find(\'{0}\').removeFromFilterByIndex({1})" title="{2}">', this.get_id(), isQuickFind ? -1 : field.Index, isQuickFind ? labelClear : _app.htmlAttributeEncode(String.format(resourcesHeaderFilter.ClearFilter, field.Label)));
        ...
        sync: function (keyValues) {
            ...
            if (keyValues) {
                ...
                for (i = 0; i < that._keyFields.length; i++)
                    row[that._keyFields[i].Index] = keyValues[i];
        ...
        _createParams: function (filterByPosition) {
            ...
            var confirmContext = that.get_confirmContext(),
                ...
                params = {
                    ...
                    ViewType: viewType, SupportsCaching: sysBrowser.agent != sysBrowser.InternetExplorer || sysBrowser.version > 7 || _app.host != null || _app.embedded,
                    ...
                    for (i = 0; i < resultFields.length; i++) {
                        ...
                        if (field.Editor)
                            if (this._executeClientEditorFactories(field)) {
                                ...
                                field.EditorId = String.format('{0}_Item{1}', this.get_id(), fieldIndex);
                        ...
                        if (field.tagged('modified-', 'created-') && field.tagged('modified-latitude', 'modified-longitude', 'modified-coords', 'created-latitude', 'created-longitude', 'created-coords')) {
                            ...
                            _geolocation = _app.host ? _app.host.geolocation : navigator.geolocation;
                            if (_geolocation) {
                                if (!geoQueue)
                                    geoQueue = _app.GeoQueue = [];
        ...
        _onExecuteComplete: function (result, context) {
            ...
            var noErrors = !result.Errors.length,
            ...
            if (pendingUploads && pendingUploads.length > 0 && noErrors && lastCommandName.match(/^(Insert|Update)$/)/* && result.RowsAffected == 1*/) {
                ...
                var upload = pendingUploads[0],
                    blobField, blobProtocolVersion = '1', blobUtilityFieldMarker;
                ...
                if (upload.files) {
                    ...
                    _app.upload('execute', {
                        ...
                        files: upload.files,
                        url: String.format('{0}Blob.ashx?{1}=u|{2}&_v={3}', __baseUrl, blobField.OnDemandHandler, key, blobProtocolVersion),
                        success: function () {
    ...
    _app.execute = function (args) {
        ...
        dataView._api = api;
        dataView.odp = args.odp === false ? false : _app.odp.get();
        if (sync)
        ...
        dataView._collectFieldValues = function () {
            ...
            if (Array.isArray(args.values))
                ...
                for (var fname in args.values)
                    values.push({
                        Name: fname,
                        NewValue: args.values[fname],
                        OldValue: null,
                        Modified: true,
                        ReadOnly: false
                    });
    ...
    */
    ...
    function filesSelected(dataView, fieldName, files) {
        ...
        function addBlobUtitlityFieldValue(newValue, type) {
            if (field) {
                fileResultValues.push({ Name: field.Name, NewValue: newValue });
                field._blobField = type + ':' + fieldName;
            }
        }
        // file name
        ...
        addBlobUtitlityFieldValue(name, 'name');
        // content type
        ...
        addBlobUtitlityFieldValue(type, 'type');
        // content length
        ...
        addBlobUtitlityFieldValue(size, 'size');
    ...
    _app.AccountManager = {
        ...
        set: function (user) {
            ...
            if (this.enabled()) {
                ...
                if (user.picture) {
                    var pictureMap = _app.storage.get('userProfilePictures');
                    if (pictureMap)
                    ...
                    //storage['userProfilePictures'] = jsonStringify(pictureMap);
                    _app.storage.set('userProfilePictures', jsonStringify(pictureMap));
                }
            }
        },
        count: function () {
        ...
        list: function (includeSession) {
            if (_touch) {
                var identities = _app.storage.get('identities');// _window.localStorage.getItem('identities');
