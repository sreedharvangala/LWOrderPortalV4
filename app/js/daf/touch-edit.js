...
* Copyright 2018 Code On Time LLC; Licensed MIT; http://codeontime.com/license
*/
...
(function () {
    ...
    function scrollGridHorizontally(dataView, elem) {
        ...
        if (grid.length) {
            var availWidth = dataView.session('grid-avail-width'),
                ...
                maxScroll = Math.ceil(availWidth - availWidth * (dataItem[0].getBoundingClientRect().width/*.width()*/ / availWidth)),
                scrollLeft = dataView.session('scroll-left') || 0,
                originalScrollLeft = scrollLeft,
                lastFixedField = dataItem.find('.app-frozen').last(),
                frameRect = _edit.frame()[0].getBoundingClientRect(),
                frameRectRight = frameRect.right,
                dataItemRect = dataItem[0].getBoundingClientRect(),
                dataItemRectRight = dataItemRect.right,
                lastFixedFieldRect = lastFixedField.length ? lastFixedField[0].getBoundingClientRect() : dataItem.find('.app-frozen-spacer')[0].getBoundingClientRect();
    ...
    function isFieldReadOnly(field) {
        ...
        if (readOnly) {
            _input._buffer = null;
            _touch.notify({ text: String.format(resourcesMobile.ReadOnly, field.HeaderText), force: true });
    ...
    _edit = _touch.edit = {
        ...
        frame: function (value) {
            ...
            if (value == 'show') {
                ...
                frame = _edit._frame = $('<span class="app-focus-frame">');
            return frame;
        },
        ...
        fieldElem: function (dataView) {
            ...
            if (dataView) {
                ...
                fieldElem = _touch.scrollable().find(containerSelector + ' .app-listview .app-selected .app-field-' + fieldName).filter(fnVisible);
            }
            return fieldElem;
        },
                    ...
                    if (fieldName) {
                        ...
                        if (fieldElem.length && (elem.closest('.ui-page-active').length || options.force)) {
                            ...
                            if (fieldName) {
                                ...
                                if (!fieldData.length)
                                    fieldData = fieldElem;
                                var rect = fieldData[0].getBoundingClientRect(),
                                    scrollableOffset = scrollable[0].getBoundingClientRect(),//.offset(),
                                    isFirstRow, dataItem,
                                    frame = _edit.frame(),
        ...
        showField: function (editorDataView, fieldName, container) {
            ...
            if (fieldName != _edit._fieldName) {
                ...
                _input.evaluate({
                    ...
                    container: container || _touch.scrollable().find('[data-input-container="' + editorDataView._id + '"]'), resize: true
                });
                ...
                if (fieldName) {
                    if (direction == 'right' || direction == 'left' || direction == 'field') {
                        ...
                        if (direction == 'down' || direction == 'up') {
                            var targetItem = elem.closest('.dv-item'),
                                newDirection;
                            if (direction == 'down')
                            ...
                            if (!isNewRow && !targetItem.is('.app-calculated') && targetItem.is(':visible')) {
                                newDirection = targetItem.find('.app-field-' + fieldName);
                        ...
                        $mobile.activePage.addClass('app-page-inlineeditor-inactive');
                ...
                if (pending.pageId == pageId) {
                    ...
                    function moveAsDirected() {
                        var moveDown = direction === 'down',
                            ...
                            fieldElem, originalField, listview, foundMatch;
        ...
        move: function (options) {
            ...
            switch (direction) {
                case 'pageup':
                case 'pagedown':
                    summaryView = _touch.scrollable().focus().find('.app-focus[data-input="dataview"]');
            ...
            if (fieldElem) {
                ...
                summaryView = _touch.summary(_touch.scrollable().find('.app-focus[data-input="dataview"]'));
                if (summaryView.length) {
            ...
            if (!result && direction == 'tab' && !$(document.activeElement).closest('[data-input]').length) {
                var dataView = _touch.dataView();
                _input.focus({ container: _touch.scrollable() });
            }
            return result;
        },
            ...
            if (dataView && !_edit._active && dataView.inlineEditing()) {
                ...
                if (allowEdit) {
                    ...
                    */
                    ...
                    _touch.notify(String.format(resourcesMobile.ReadOnly, text));
                }
            }
            return result;
        },
        expandBy: function (textInput, delta) {
            ...
            if (overlayPage.length) {
                ...
                targetScrollableRect = targetScrollable[0].getBoundingClientRect();

                scrollable = overlayPage.find('.app-wrapper');
                scrollableRect = scrollable[0].getBoundingClientRect();
                ...
                overlayPage.css({ minWidth: overlayWidth, maxWidth: overlayWidth });
                overlayRect = overlayPage[0].getBoundingClientRect();
        ...
        form: function (dataView) {
            ...
            layout.push(
                ...
                '<i class="app-icon material-icon material-icon-more app-btn-more" title="' + resourcesMobile.More + '"/>'
            );
            dataView._fields.forEach(function (f) {
                var fieldName = f.Name;
                if (f.Type != 'DataView')
                    layout.push(
                        ...
                        '<div class="app-form-inlineeditor-label"><span data-control="label" data-field="' + fieldName + '"><span class="app-control-inner"/></span></div>' +
                        '<span data-control="field" data-field="' + fieldName + '" data-auto-expand="true"><span class="app-control-inner"/></span>' +
                        '</div>');
            });
            layout.push(
                '</div>' +
        ...
        position: function (page) {
            ...
            if (changePosition) {
                ...
                if (overlayDisplayMode) {
                    page.addClass('app-page-inlineeditor-overlay').toggleClass('app-page-inlineeditor-textalignright', fieldElem.css('text-align') == 'right');
                    ...
                    if (fieldElem) {
                        var frameRect = fieldElem[0].getBoundingClientRect(),
                            pageRect = page[0].getBoundingClientRect(),
    ...
    $(document).on('dataitemselect.dataview.app', function (e) {
        if (e.namespace == 'app.dataview' && _edit._dataItemSelect != false) {
            ...
            if (dataView.inlineEditing()) {
                ...
                //if (e.action != 'select')
                _edit.sync({ dataView: dataView, elem: fieldElem, scrollIntoView: true, ignoreOtherInputs: true });
        ...
        if (e.namespace == 'app' && !_touch.isInTransition())
            ...
            if (e.namespace == 'app' && !_edit._active) {
                ...
                else if (_edit.move({ direction: e.direction, originalEvent: originalEvent }))
                    return false;
            }
            ...
            if (_edit.frame(':active')) {
                ...
                if ((key.length == 1 || key.match(/Backspace|Del|Delete/)) && !(originalEvent.ctrlKey || originalEvent.altKey || originalEvent.metaKey)) {
                    _input._buffer = key;
                    originalEvent.preventDefault();
                    setTimeout(_edit.activate); // do not activate in the "keydown" handler
                }
            }
            ...
            if (fieldElem) {
                ...
                var scrollableRect = _touch.scrollable()[0].getBoundingClientRect();
                $mobile.activePage.find('.app-listview .app-selected .app-btn-more').filter(fnVisible).each(function () {
            ...
            if (_touch.pointer('touch') && e.type == 'vclick')
                _edit.activate();
            else
                _focusFrameTimeout = setTimeout(function () {
