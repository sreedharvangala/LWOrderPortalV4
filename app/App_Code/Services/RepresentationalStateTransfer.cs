...
namespace Finsoft.Services
{
        ...
        protected virtual void CombineScripts(HttpContext context, bool isSaaS, string scriptName, string culture, string version)
        {
            ...
            if (isSaaS)
            {
                if (ApplicationServices.IsTouchClient)
                	...
                	sb.AppendFormat(string.Format("$(\'<link></link>\').appendTo($(\'head\')).attr({{ href: \'{0}/App_Themes/Finsoft/_The" +
                                "me_Vantage.css?{0}\', type: \'text/css\', rel: \'stylesheet\' }});", ApplicationServices.Version), baseUrl);
                try
                {
                ...
                sb.AppendFormat("var __targetFramework=\'4.6\';__tf=4.0;__cothost=\'appfactory\';__appInfo=\'OrderPorta" +
                        "lV4|{0}\';", BusinessRules.JavaScriptString(context.User.Identity.Name));
        ...
        protected virtual void BeginResponsePadding(HttpRequest request, StreamWriter sw)
        {
            ...
            if (!(string.IsNullOrEmpty(callback)))
            	...
            	if ((request.HttpMethod == "GET") && UriRestConfig.IsJSONPRequest(request))
                {
                    ...
                    sw.Write("Finsoft=typeof Finsoft==\'undefined\'?{{}}:Finsoft;Finsoft.{0}=", instance);
                }
        }
    ...
    public partial class MSGraphOAuthHandlerBase : OAuthHandler
    {
        ...
        public override void SetUserAvatar(MembershipUser user)
        {
            if (ApplicationServices.IsSiteContentEnabled)
            	try
                {
                    var request = WebRequest.Create("https://graph.microsoft.com/beta/me/photo/$value");
                    request.Headers[HttpRequestHeader.Authorization] = ("Bearer " + ((string)(Tokens["access_token"])));
    ...
    public partial class LinkedInOAuthHandlerBase : OAuthHandler
    {
        ...
        protected override string Scope
        {
            get
            {
                return (("r_basicprofile r_emailaddress " + Config["Scope"])).Trim();
            }
        }
        ...
        public override string GetUserName()
        {
            _userObj = Query("people/~:(id,first-name,last-name,picture-urls::(original),industry,positions,ema" +
                    "il-address)", false);
            return ((string)(_userObj["emailAddress"]));
        }
        
        public override string GetUserImageUrl(MembershipUser user)
        {
            var pictures = _userObj["pictureUrls"];
            if ((pictures != null) && (pictures is JObject))
            {
                var values = ((JArray)(pictures["values"]));
                if (values.Count > 0)
        ...
        protected override WebRequest GetQueryRequest(string method, string token)
        {
            var request = WebRequest.Create(("https://api.linkedin.com/v1/" + method));
            request.Headers[HttpRequestHeader.Authorization] = ("Bearer " + token);
