    ...
    public partial class DataControllerBase : IDataController, IAutoCompleteManager, IDataEngine, IBusinessObject
    {
        ...
        public void ProcessManyToManyField(string controllerName, DataField field, FieldValue fieldValue, object primaryKey)
        {
            ...
            if ((args != null) && args.IsBatchEditOrDelete)
            {
                ...
                if (p.Rows.Count == 1)
                {
                    ...
                    fieldValue.OldValue = p.Rows[0][0];
                }
        ...
        public virtual string GetSurvey(string surveyName)
        {
            ...
            if (string.IsNullOrEmpty(survey))
            	survey = ControllerConfigurationUtility.GetResourceText(string.Format("Finsoft.Surveys.{0}.min.js", surveyName), string.Format("Finsoft.{0}.min.js", surveyName), string.Format("Finsoft.Surveys.{0}.js", surveyName), string.Format("Finsoft.{0}.js", surveyName));
            if (string.IsNullOrEmpty(survey))
            	throw new HttpException(404, "Not found.");
            if (string.IsNullOrEmpty(layout))
            	layout = ControllerConfigurationUtility.GetResourceText(string.Format("Finsoft.Surveys.{0}.html", surveyName), string.Format("Finsoft.Surveys.{0}.htm", surveyName), string.Format("Finsoft.{0}.html", surveyName), string.Format("Finsoft.{0}.htm", surveyName));
            if (string.IsNullOrEmpty(rules))
            	rules = ControllerConfigurationUtility.GetResourceText(string.Format("Finsoft.Surveys.{0}.rules.min.js", surveyName), string.Format("Finsoft.{0}.rules.min.js", surveyName), string.Format("Finsoft.Surveys.{0}.rules.js", surveyName), string.Format("Finsoft.{0}.rules.js", surveyName));
        ...
        protected virtual bool ValidateViewAccess(string controller, string view, string access)
        {
            if (!ApplicationServicesBase.AuthorizationIsSupported)
            	return true;
            var context = HttpContext.Current;
            if ((AllowPublicAccess || FullAccessGranted(controller)) || (context.Request.Params["_validationKey"] == BlobAdapter.ValidationKey))
            	return true;
        ...
        DataTable ExecuteResultSetTable(ViewPage page)
        {
            ...
            if ((fieldFilter != null) && (fieldFilter.Length > 0))
            {
                ...
                {
                    var fieldName = page.Fields[fieldIndex].Name;
