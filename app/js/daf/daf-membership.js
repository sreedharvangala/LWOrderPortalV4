    ...
    _wm.prototype = {
        ...
        showPermalink: function () {
            if (!this._savePermalinkParams || String.isNullOrEmpty(this._savePermalinkParams.html)) {
                _app.alert(_resources.Messages.PermalinkUnavailable);
                return;
            }
        ...
        _showHistory_Success: function (result) {
            if (result.length == 0) {
                _app.alert(_resources.Messages.HistoryUnavailable)
                return;
            }
        ...
        loggedIn: function () {
            ...
            var loggedIn = _app.loggedIn(); // supportsIsLoggedIn ? Sys.Services.AuthenticationService.get_isLoggedIn() : true;
        ...
        _login_Completed: function (validCredentials) {
            ...
            if (!validCredentials) {
                var that = this;
                _app.alert(_resources.Messages.InvalidUserNameAndPassword, function () {
                    if (!_touch)
        ...
        login: function () {
            ...
            if (this._userName && this._password) {
                ...
                if (isBlank.exec(this._userName.value)) {
                    _app.alert(_resources.Messages.BlankUserName, function () {
                ...
                if (isBlank.exec(this._password.value)) {
                    ...
                    _app.alert(_resources.Messages.BlankPassword, function () {
                        that.set_isLoginDialogVisible(false);
