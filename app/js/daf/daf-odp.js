...
(function () {
            ...
            if (parent) {
                ...
                */
                if (_app.odp.enabled()) {
                    ...
                    if (odp !== false && methodName.match(/GetPage|GetListOfValues|Execute/)) {
            ...
            if (prefetchJSON) {
                ...
                if (runOnServer) {
                    var ensureData = (methodName === 'GetPage' || methodName === 'Execute') && !(dataView._api && dataView._api.background),
                        ...
                        startUrl = _app.touch ? _app.touch.startUrl() : locationHref;
            ...
            if (missing.length)
                if (_odp.offline('sync', { controller: missing }))
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        dataType: 'text',
                        data: JSON.stringify({ controllers: controllers })
    ...
    _app.OfflineDataProcessor.prototype = {
        ...
        invoke: function (data) {
            ...
            function commitToOffline() {
                var log = that._log.slice(0);
                if (masterChanged)
                    log.splice(0, 0, packageActionArgs(that._date));
                if (log.length)
                    _odp.offline('commit', { dataView: dataView, data: that._data, log: log });
            }
            ...
            if (isRoot) {
                if (method === 'GetPage' && dataView.get_lastCommandName() === 'New')
                    ...
                    masterChanged = commandName === 'Insert';
                    if (!masterChanged)
                    ...
                    if (_odp.offline('sync', { controller: controller })) {
                        if (commandName !== 'Update' || masterChanged) {
                            masterChanged = true;
                            executeOnClient();
                            ...
                            commitToOffline();
                            data.deferred.resolve(jsonToData(ActionResult_new(1)));
                        }
                        // commit to offline storage on the client
                        return true;
                    }
                    else if (commandName.match(/^(Insert|Update)$/)) {
                        if (log.length || masterChanged && odpSettings.commit) {
                            // commit the log 
                            if (masterChanged)
                                log.splice(0, 0, packageActionArgs(that._date));
                            ...
                            if (commandName === 'Update' && odpSettings.commit) {
        ...
        execute: function (options) {
            ...
            if (method === 'Execute') {
                ...
                if (updating || deleting) {
                    ...
                    that.select({ from: controllerName, where: { filter: keyFilter.join(''), params: params }, limit: deleting ? Number.MAX_VALUE : 1, remove: deleting }).done(function (result) {
                        ...
                        actionResult.RowNotFound = !result.length;
