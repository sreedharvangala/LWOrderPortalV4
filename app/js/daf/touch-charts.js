...
(function () {
        ...
        if (packageInfo && packageInfo.loaded)
            ...
            if (packageInfo && !packageInfo.loaded)
                ...
                google.charts.load('current', {
                    packages: [packages], mapsApiKey: $app.touch.mapsApiKey(), callback: function () {
                        $app.touch.busyIndicator(false);
                        packageInfo.loaded = true;
                        $(packageInfo.callbacks).each(function () {
                            this();
    ...
    $app.touch.presenter('register', {
        ...
        show: function (options) {
            ...
            function render() {
                touch.registerAPI('Charts', renderWithApi);
            }
            ...
            function renderWithApi() {
                ...
                $app.touch.resetPageHeight();
                touch.syncEmbeddedViews(scrollable);
                if ($app.touch.pointer() == 'mouse')
...
(function () {
    ...
    function setDateFraction(date, hour, frac) {
        var precision = touch.settings('calendar.drag.precision') || 15,
    ...
    $(document)
        ...
        .on('scroll.app scrollstop.app resized.app', function (e) {
            ...
            if (resized) {
                ...
                header.find('.app-tabs').css('visibility', header.width() < 768 ? 'hidden' : 'visible');
            }
            ...
            scrollStopTimeout = setTimeout(function () {
                ...
                if (!isScrolling && (resized || !isDayWeek)) {
                    fetchCalendarData(e.relatedTarget, resized);
                }
            }, 300);
        ...
        .on('sidebarstatechanged.app', function () {
            refreshCalendar($app.touch.contextDataView());
        });
    ...
    function refreshCalendar(dataView) {
        ...
        if (!supportsCalendar(dataView))
            ...
            if (dataView.calendarPlugin) {
                if (__settings.bars.left.mini || $.mobile.activePage.is('.app-page-modal'))
                    detachPlugin(sidebar);
                else
        ...
        show: function (options) {
            ...
            if (calendar.length > 0) {
                ...
                if (!isEcho) {
                    ...
                    $app.touch.tabs('fit', { container: header });
                ...
                if (!isEcho) {
                    ...
                    var tabs = $('<div class="app-tabs"></div>').appendTo(header);
        ...
        updateHeader: function (header) {
            ...
            updateHeaderTimeout = setTimeout(function () {
                var scrollable = that.scrollable(),
                    ...
                    tabVisible = appTabs.css('visibility') != 'hidden' && !appTabs.is(':empty'),
        ...
        _scroll: function (view, header, distance, footer, animate) {
            ...
            shiftViewTimeout = setTimeout(function () {
                ...
                if (distance < colWidth) {
                    ...
                    var newDayHeaders = $(that.calendar.drawDayHeaders(date, drawDistance)).width(colWidth / 7).prependTo(header.find('ul'));
    ...
    var CalendarInput = touch.CalendarInput = (function (method, options) {
        ...
        var input = options.input,
            ...
            field = options.field = options.field || (inputContainer && $app.input.elementToField(inputContainer)),
        ...
        function attach() {
            ...
            if (!outerContainer.length) {
                outerContainer = options.container = $('<div class="app-calendar-plugin-container app-data-input-helper"></div>').hide().appendTo($body);
                $('<div class="ui-btn ui-btn-icon-notext app-calendar-btn-close material-icon"></div>').appendTo(outerContainer).attr('title', resources.ModalPopup.Close);
            }
            if (!cover.length)
                cover = $('<div class="app-calendar-cover" style="display:none"></div>').appendTo($body);
        ...
        function focus(skipTouchCheck) {
            ...
            if (touchPointer && !skipTouchCheck) {
                $('.app-data-input').blur();
            ...
            var wrapper = inputContainer.closest('.app-wrapper'),
                ...
                center = windowWidth <= 480 && windowHeight <= 480,
                modal = touchPointer || center;

            if (options.date)
                outerContainer.data('select-date', options.date);

            if (touchPointer && windowWidth < windowHeight)
                outerContainer.removeClass('app-calendar-wide');
            else
                outerContainer.addClass('app-calendar-wide');
            ...
            if (!center) {
                ...
                left = inputPos.left;
                // check right
                if (left + containerWidth - windowWidth > 0)
                    left = windowWidth - containerWidth - 10; // shadow
                ...
                if (touchPointer) {
                    ...
                    if (top + containerHeight > windowHeight) {
                        // position above
                        top -= containerHeight + inputContainer.outerHeight();
                        if (top < 0)
                            center = modal = true;
                    }
                }
            ...
            if (modal) {
                ...
                cover.hide();
                outerContainer.removeClass('app-calendar-show-bar');
            }
        ...
        function setInput(dataView, field, input, date) {
            ...
            if (touchPointer || !input || input.parent().length == 0) {
                ...
                if (!touchPointer)
                    setTimeout(function () {
                        CalendarInput.ignoreShow = true;
                        $app.input.focus({ dataView: dataView, fieldName: field.Name });
                    }, 50);
            }
            else {
        ...
        switch (method) {
            ...
            case 'escape':
                ...
                if (options.container.is('.app-calendar-show-bar')) {
                    CalendarInput.ignoreShow = true;
                    $app.input.focus({ fieldName: options.field.Name });
                }
                break;
            case 'focus':
                focus();
                break;
            case 'activate':
                if (touchPointer || $window.width() <= 640 && $window.height() <= 640) {
                    attach();
                    return focus();
                }
                return false;
    ...
    $(document)
        ...
        .on('input', '.app-data-input[data-datepicker-attached]', function (e) {
            var input = $(e.target),
                field = $app.input.eventToField(e),
                dv = field._dataView,
    ...
    var CalendarControl = (function (method, options) {
        ...
        function refresh() {
            ...
            // get values in month
            if (reloadData && !options.hideDate && !dataView._survey && field.AllowQBE && !(dataView._keyFields.length == 1 && dataView._keyFields[0].Name == 'sys_pk_')) {
            ...
            if (createContainer) {
                ...
                if (options.renderTime) {
                    ...
                    if (!options.hideActionBar) {
                        ...
                        if (touch.settings('ui.actions.reverse') == true)
    ...
    CalendarPlugin.prototype = {
        attach: function (sidebar) {
            ...
            if (!calendarExisted) {
                var fieldSelector = $('<a class="ui-btn ui-btn-icon-none app-has-droparrow app-calendar-plugin-fieldselector"><span class="app-icon material-icon" style="opacity:0">filter_list</span><span class="text">' + activeField.Label + '</span></a>').appendTo(calendar),
    ...
    $(document)
        ...
        .on('beforeactivatedatetime.input.app beforeactivatedate.input.app', '[data-input]', function (e) {
            if (e.namespace == 'app.input') {
                ...
                options.field = $app.input.elementToField(options.inputContainer);
